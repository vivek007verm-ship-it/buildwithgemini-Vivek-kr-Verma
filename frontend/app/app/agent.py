...
discovery_agent = Agent(
    ...
    instruction="Search and filter the book catalog in Firestore, fetch live external details/cover images using fetch_external_book_details, generate custom book art using generate_book_illustration, and find nearby libraries/bookstores. Always embed returned cover or generated image URLs in markdown as ![Cover / Illustration](image_url). Never claim you cannot display or generate images.",
    tools=[search_books_firestore, search_book_catalog, fetch_external_book_details, generate_book_illustration, geocode_address, find_nearby_places, load_memory, preload_memory],
)
...
When the user asks for covers or images, call fetch_external_book_details or generate_book_illustration and embed returned image URLs directly as markdown images ![Book Cover](url).""",
    tools=[load_memory, preload_memory, search_books_firestore, save_book_to_firestore, fetch_external_book_details, generate_book_illustration, search_public_poetry],
)
...
companion_agent = Agent(
    ...
    instruction="Provide warm reading companion tips, atmosphere suggestions, custom book illustrations via generate_book_illustration (always embed returned image URLs as markdown ![Illustration](url)), poetry/literary excerpts, and nearby library/bookstore recommendations. Strictly respect all remembered user allergies (e.g. avoiding dust/old books or allergen-containing snacks/teas).",
    tools=[load_memory, preload_memory, search_public_poetry, generate_book_illustration, geocode_address, find_nearby_places],
)
